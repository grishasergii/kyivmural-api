...
def test_post_request_new_mural_creates_mural(fake_environment, murals_table):
    ...
    response = murals_table.get_item(Key={"id": "test-id", "artist_name_en": "some artist"})
...
def test_post_request_new_mural_without_artist_name_creates_mural(fake_environment, murals_table):
    event = {
        "requestContext": {
            "httpMethod": "POST"
        },
